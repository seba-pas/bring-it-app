...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (createdFavorite) return res.send(`Favorito agregado correctamente`);
      ...
      return res.send(
        `El producto con id: ${productId} ya se encuentra marcado como favorito para el usuario ${userEmail}`
      );
  } catch (error) {
...
router.delete("/", async (req, res) => {
  ...
  try {
    ...
    return res.send(
      `Producto de id ${productId} del usuario ${userEmail} no pertenece a favoritos`
    );
  } catch (error) {
    return res.send(error);
  }
